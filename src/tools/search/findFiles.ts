...
export const findFilesTool: Tool = {
  definition: {
    name: "findFiles",
    description:
      "Find files in the workspace using a glob pattern. Use **/ prefix to search recursively (e.g., **/index.ts)",
    inputSchema: {
      type: "object",
      properties: {
        pattern: {
          type: "string",
          description:
            "The glob pattern to search for. Use **/ for recursive search (e.g., **/index.ts, **/*.tsx, src/**/*.ts)",
        },
        maxResults: {
          type: "number",
  ...
  execute: async (args: unknown, ctx: ToolContext): Promise<ToolOutput> => {
    const { pattern: inputPattern, maxResults: inputMaxResults } = args as {
      pattern: string;
      maxResults?: number;
    };
