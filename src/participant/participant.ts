    ...
    const participant = vscode.chat.createChatParticipant(
        ...
        async (request, _context, stream, token) => {
            ...
            try {
                    ...
                    for (const toolCall of toolCalls) {
                        ...
                        if (tool) {
                            try {
                                ...
                                outputChannel.appendLine(`[${new Date().toISOString()}] Tool ${toolCall.name} result length: ${result.text.length}`);
                            } catch (err: unknown) {
                                const errorMessage = err instanceof Error ? err.message : String(err);
                                result = { text: `Error executing tool: ${errorMessage}` };
                                stream.markdown(`❌ **Error:** ${errorMessage}\n`);
                                outputChannel.appendLine(`[${new Date().toISOString()}] Tool Error: ${errorMessage}`);
                            }
                        } else {
                ...
                outputChannel.appendLine(`[${new Date().toISOString()}] Error in chat loop: ${errorMessage}`);
            }
        }
    );
