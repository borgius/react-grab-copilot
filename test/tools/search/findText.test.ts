...
describe("findTextTool", () => {
  ...
  it("should find text using grep", async () => {
    ...
    const result = await findTextTool.execute(
      { query: "search term" },
      mockCtx,
    );
  ...
  it("should include pattern in grep command", async () => {
    ...
    await findTextTool.execute(
      { query: "search term", includePattern: "*.ts" },
      mockCtx,
    );
  ...
  it("should throw error on failure", async () => {
    ...
    await expect(
      findTextTool.execute({ query: "search term" }, mockCtx),
    ).rejects.toThrow("Grep error: Execution failed");
  });
